...
class devNode{
    ...
    backoff() {
        // 简化的退避机制：随机等待一段时间
        const backoffTime = Math.random() * 100; // 随机退避时间（单位：tick）
        console.log(`Node ${this.sNo} is backing off for ${backoffTime.toFixed(2)} ticks.`);
        // 在退避时间后再次尝试发送
        setTimeout(() => {
            this.capBehavior(); // 重新进入 CAP 行为
        }, backoffTime * SIMU_TICK);
    }
...
function selectNextNode(currentNode, possibleNodes, alpha, beta)
 {
    ...
    possibleNodes.forEach(node => {
        totalProbability += calculateTransitionProbability(currentNode, node, alpha, beta);
    });

    let probabilities = possibleNodes.map(node => {
        return calculateTransitionProbability(currentNode, node, alpha, beta) / totalProbability;
    });
