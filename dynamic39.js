...
const ALL_STEPS_THESHOLD=1000000000;  //仿真总步数
...
class devNode{
    constructor(canvas,sNo,pwrRad,x,y,srcVolumn,nodeNum){
        this.sNo=sNo;   //仿真时默认已有编号，真实运行时除1号以外，先置30去申请编号
        this.id=0;      //编号累加，中间节点用来识别此包是否已被转发过
        this.longName='64byteName_'+sNo.toString();
        this.volumn=srcVolumn;  //高音量节点屏蔽低音量节点

        this.justRecvedPacks=[];  //最近收到的数据包,成员：[src,id,存在时间]
        this.isSrc=false;
        this.nPermittedSend=0;    //每次发送后，置NO_SEND表示不可发送，逐渐减到0表示可发送
        this.srcSendDelay=0;
        this.waitSendFifo=[];     //等待发送的数据包队列[msg包,存在时间]作为成员
        
        this.handshaked=0;        //发完数据，置T_HANDSHAKE，逐渐减到0产生一个控制包短包

        this.neighborsRssiTtl=[];  //nodeNum=pub_totle_nodes项[rssi,ttl]
        for(let i=0;i<nodeNum;i++)
            this.neighborsRssiTtl.push([0.0,0]);
        
        this.data=[];   //当前包的数据
        let base=this.generateData();
        this.data=base;

        this.downloadData=[]; //接收到的数据，累积
        ...
        this.speed = 0; // 节点移动速度，初始为0
        this.angle = 0; // 移动方向角度，初始为0
    ...
    drawNode(type)
    {
        ...
        if(type=='all' || type=='core')
        {
            ...
            this.ctx.beginPath();
            this.ctx.fillStyle=this.color;
            this.ctx.font="16px Arial";  //Georgia
            this.ctx.fillText(this.sNo.toString()+'/'+this.maxHop.toString()+'/'+(this.sendNum-this.sendHandNum).toString()+'/'+this.recvDataNum.toString(), this.x, this.y-14); 
        }
    }
...
function myNeighborSending(x0,y0,rad){  //邻居发送时，当前节点不能发送
    let ret=false;
...
function atPackDesEnd(msg,nodeSNo,nodeNeighbors){
    let end=true;  //当前节点置1，若des为0但邻居为1，则将对应0置1，然后发送；否则结束转发
                    
    let len=nodeNeighbors.length;
    msg.des[nodeSNo-1]=1;
    for(let i=0;i<len;i++)
    {
        let [rssi,a]=nodeNeighbors[i];
        if(msg.des[i]==0 && rssi>0)
        {
            msg.des[i]=1;
            end=false;
        }
...
function doSimu()
{
    ...
    setTimeout(function stepSimu(){
        ...
        for(let j=0;j<pub_totle_nodes;j++)
        {
            ...
            {
                if(node.isSrc && node.id<ALL_DATA_PACK_NUM && tickTotle>=DELAY_FOR_HANDSHAKE)        
                {
            ...
            for(let i=0;i<lenRTQ;i++)
            {
                ...
                if(b){
                    node.setANeighbor(rtItem.lastNode,rss,pub_totle_nodes);
        ...
        if(tickTotle>=DELAY_FOR_HANDSHAKE)
            noSignal++;
        let lenRTQ1=rxtxQue.length;  //连续40个节拍，无空中数据信号则终止仿真
        for(let i=0;i<lenRTQ1;i++)
        {
            if(rxtxQue[i].type==0)
                noSignal=0;            
        }
        ...
        if(noSignal>160)
        {
            alert('所有可传数据已传送完毕，仿真结束');
            showResult();
            return;
        }

        setTimeout(stepSimu, SIMU_TICK);   //实际应该是20ms，即每次能够容纳1个802.15.4基础超帧15.36ms
    }, SIMU_TICK);
...
function showResult(){  //显示仿真结果、统计
    let result=document.getElementById('id-textaera-result');
    result.innerHTML='';
    for(let i=0;i<pub_totle_nodes;i++)
    {
        result.innerHTML+=(i+1).toString()+':\n';
        result.innerHTML+='   总发送次数'+Nodes[i].sendNum.toString()+'\n';
        result.innerHTML+='   源发送次数'+Nodes[i].sendSrcNum.toString()+'\n';
        result.innerHTML+='   握手发送次数'+Nodes[i].sendHandNum.toString()+'\n';
        result.innerHTML+='   转发次数'+Nodes[i].sendTransNum.toString()+'\n';
        result.innerHTML+='   最大跳数'+Nodes[i].maxHop.toString()+'\n';
        result.innerHTML+='   收到数据'+Nodes[i].downloadData.length.toString()+'\n';
        result.innerHTML+='   邻居\n';
        let s='';
        for(let j=0;j<pub_totle_nodes;j++)
        {
            if(Nodes[i].neighborsRssiTtl[j][0]>0)
                ...
                s+='0';
            if(j%4==3 && j!=pub_totle_nodes-1)
                s+='_';
        }
        result.innerHTML+='   '+s+'\n';
        result.innerHTML+='   数据\n';
        s='';
        let len=Nodes[i].downloadData.length;
        for(let j=0;j<len;j++)
        {
            s+=Nodes[i].downloadData[j].toString();
            if(j!=len-1)
                s+=',';
        }
        result.innerHTML+=s+'\n';
